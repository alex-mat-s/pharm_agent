...
Откроется на `http://127.0.0.1:7860`. Интерфейс содержит 7 вкладок:

1. **Новый анализ** — МНН, заболевание, регион, стадия, 2 PDF → создание run и обогащение.
2. **Верификация** — обязательное одобрение/отклонение результатов обогащения.
3. **Прогресс** — запуск Scientific Agent и пошаговый статус (PubMed, ClinicalTrials, FDA, EMA, PDF, синтез).
4. **Evidence** — таблица нормализованных источников с фильтрацией по типу.
5. **Scientific Memo** — просмотр и скачивание научного memo из Obsidian.
6. **Market Memo** — просмотр рыночного анализа (конкуренты, ценообразование, коммерческие риски).
7. **Логи** — audit events для текущего run (секреты маскируются).
8. **Healthcheck** — проверка доступности всех компонентов системы.

### CLI: Основной запуск (end-to-end flow)
...
1. Работа с фиксированным числом pdf
2. Поиск по ограниченному числу сайтов